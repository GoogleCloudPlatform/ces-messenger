...
Explicitely set the session ID instead of letting the widget generate a new one. This can be useful in cases where you want to continue in the widget a session that was initiated somewhere else (e.g. phone call).

This function must be called as early as possible to ensure that the widget has the provided session ID before establishing the connection. Example:

```javascript
window.addEventListener('ces-messenger-loaded', () => {
  const cesMessenger = document.querySelector('ces-messenger');
  cesMessenger.setSessionId(`custom-session-id-${Date.now()}`);
});
```

Please note that:

1. If you try to use a session ID that has already been used for a session that has terminated, the connection will fail.

2. Tokens generated by the `MANAGED` token broker are tied to the session ID. You cannot reuse a token in two different sessions.
